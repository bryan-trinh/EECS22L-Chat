...
int ChatRecieve(int client_fd, int message_size, char client_username[10][64], char* message);
int Exit = 0;
...
/*************************************************** LOT OF STUFF TO SUPPORT ***********************************************************/
							...
							fd_location = ChatRecieve(i, sizeof(buffer), client_username, message);
...
int ChatRecieve(int client_fd, int message_size, char client_username[10][64], char* message) {
	...
	char discard[MSG_LEN] = {0};
		...
		recv(client_fd, username, message_size, 0);         /*get username */   
		...
		printf("The username sent to is: %s \n", username);
		...
		if(username_found && password_found) {
			...
			if(!already_logged_in) {
				...
				if(full) {
					...
					read(client_fd, discard, MSG_LEN);
					write(client_fd, "failed", strlen("success"));
					read(client_fd, discard, MSG_LEN);
